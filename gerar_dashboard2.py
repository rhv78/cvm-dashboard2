...
Regra de datas:
- Cards individuais: cada fundo mostra sua ultima data REAL
  (fundos com cota de maio aparecem com 05/05, fundos parados em 30/04
  aparecem com 30/04).
- Consolidado, Carteira Total, grafico, CDI e pizza: travados na
  ultima data COMUM a todos os fundos (menor dt_fim entre os fundos),
  para nao misturar patrimonio em datas diferentes.
"""
...
# Divisor usado em cotas_qt = patrim / (cota * 1000). Todo valor exibido como
# "patrimonio" ja esta nessa escala; os resgates precisam usar a MESMA escala,
# senao apareceriam 1000x maiores que o patrimonio ao lado.
ESCALA = 1000.0
...
# ── RESGATES ──────────────────────────────────────────────────────────────────
# RESG_DIA do informe diario = resgates do dia do FUNDO INTEIRO (todos os
# cotistas), nao a sua posicao. A coluna so existe aqui se o coletar_cotas2.py
# a mantiver no CSV consolidado; se nao existir, o dashboard mostra "n.d."
# em vez de fingir zero.
COL_RESG = next((c for c in ("RESG_DIA", "VL_RESG", "RESG") if c in df.columns), None)
TEM_RESG = COL_RESG is not None

if TEM_RESG:
    df["RESG_F"] = pd.to_numeric(
        df[COL_RESG].astype(str).str.replace(",", "."), errors="coerce"
    ).fillna(0.0)
    print(f"  Resgates: coluna '{COL_RESG}' encontrada.")
else:
    df["RESG_F"] = 0.0
    print("  AVISO: coluna RESG_DIA ausente no CSV consolidado.")
    print("         Os resgates aparecerao como 'n.d.' no dashboard.")
    print("         Para habilitar, mantenha RESG_DIA no coletar_cotas2.py.")
...
for cnpj_raw, nome in CNPJS.items():
    ...
    # df_f ja passou por drop_duplicates("DT_COMPTC"), entao nao ha risco de
    # somar o mesmo dia duas vezes.
    ...
    for dt in datas_todas:
        ...
        hist.append({
            ...
            "resgate": round((resg_map.get(dt, 0.0) or 0.0) / ESCALA, 2),
        })
    ...
    resg_fundo = round(sum(h.get("resgate", 0.0) for h in hist), 2)
    ...
    print(f"      Rent: {rent_final:+.4f}%  |  Patrim: R$ {patrim_ini:,.2f} -> R$ {patrim_fim:,.2f}  |  Resultado: R$ {ganho:+,.2f}  |  ate {dt_fim}")
    print(f"      Resgates no periodo: {('R$ %s' % format(resg_fundo, ',.2f')) if TEM_RESG else 'n.d.'}\n")
...
# ──────────────────────────────────────────────────────────────────────────────
# DATA COMUM (so para consolidado / carteira / CDI / grupos / pizza)
# Cards individuais NAO sao alterados — cada fundo mantem sua data real.
# ──────────────────────────────────────────────────────────────────────────────
ultima_dt = min(f["dt_fim"] for f in fundos)
print(f"  Data limite comum (consolidado): {ultima_dt}")
print(f"  (cards individuais mantem sua data real)\n")

# ── TOTAIS ────────────────────────────────────────────────────────────────────
# tot_fim = soma do patrimonio de cada fundo NA data comum
# (nao o patrim_fim individual, que pode ser de datas diferentes)
tot_ini = sum(f["patrim_ini"] for f in fundos)

tot_fim = 0.0
for f in fundos:
    h = next((x for x in reversed(f["historico"]) if x["dt"] <= ultima_dt), None)
    if h: tot_fim += h["patrimonio"]
tot_fim   = round(tot_fim, 2)
...
# Resgates consolidados: travados na data comum, igual ao resto do consolidado.
tot_resg = round(sum(
    h.get("resgate", 0.0)
    for f in fundos for h in f["historico"] if h["dt"] <= ultima_dt
), 2)
print(f"  RESGATES (ate {ultima_dt}): {('R$ %s' % format(tot_resg, ',.2f')) if TEM_RESG else 'n.d.'}\n")
...
# ── CARTEIRA CONSOLIDADA (TWR — ajustada por aportes/resgates) ────────────────
# Em vez de (patrim_fim / patrim_ini - 1), que se distorce quando ha resgates,
# usamos Time-Weighted Return: em cada data, isolamos o fluxo (variacao de
# cotas_qt entre um dia e outro do mesmo fundo, valorada pela cota do dia)
# e calculamos o retorno do dia como:
#     ret_dia = (patrim_t - fluxo_t) / patrim_{t-1} - 1
# O acumulado eh o produto de (1 + ret_dia). Isso da a rentabilidade REAL
# da carteira, independente de entradas/saidas de dinheiro.

# Tolerancia para ignorar mudancas de arredondamento nas cotas reportadas
EPS_COTAS = 0.001

# Mapa {cnpj -> {dt -> registro historico}} para consulta rapida
hist_map = {f["cnpj_raw"]: {h["dt"]: h for h in f["historico"]} for f in fundos}

def snapshot_carteira(dt_atual, dt_anterior):
    """
    Retorna (patrim_hoje, patrim_ontem_mesma_composicao, fluxo).
    - patrim_hoje: soma dos patrimonios dos fundos em dt_atual
      (ou ultimo disponivel <= dt_atual)
    - patrim_ontem_mesma_composicao: patrim em dt_anterior usando as MESMAS
      quantidades de cotas de hoje (para o retorno ser puramente por cota)
    - fluxo: variacao de cotas_qt entre dt_anterior e dt_atual, valorada
      pela cota do dia (positivo = aporte, negativo = resgate)
    """
    pat_hoje = 0.0
    pat_ontem_ajustado = 0.0
    fluxo = 0.0
    for f in fundos:
        # Ultimo registro <= dt_atual
        h_hoje = None
        for x in reversed(f["historico"]):
            if x["dt"] <= dt_atual:
                h_hoje = x; break
        if h_hoje is None: continue

        # Ultimo registro <= dt_anterior
        h_ontem = None
        for x in reversed(f["historico"]):
            if x["dt"] <= dt_anterior:
                h_ontem = x; break

        pat_hoje += h_hoje["patrimonio"]

        if h_ontem is None:
            # Fundo nao existia em dt_anterior — trata como aporte completo
            fluxo += h_hoje["cotas_qt"] * h_hoje["cota"]
            continue

        # Detectar variacao de cotas (aporte/resgate)
        d_cotas = h_hoje["cotas_qt"] - h_ontem["cotas_qt"]
        if abs(d_cotas) < EPS_COTAS:
            d_cotas = 0.0
        fluxo += d_cotas * h_hoje["cota"]

        # Patrimonio de ontem com as cotas de ontem (pre-fluxo)
        pat_ontem_ajustado += h_ontem["cotas_qt"] * h_ontem["cota"]

    return round(pat_hoje, 2), round(pat_ontem_ajustado, 2), round(fluxo, 2)

# Datas efetivamente usadas na carteira: 31/12 + todas as datas ate ultima_dt
datas_cart = [d for d in datas_todas if d <= ultima_dt]

cart_hist = [{
    "dt": "2025-12-31",
    "patrimonio": round(tot_ini, 2),
    "rent_acum": 0.0,
    "var_diaria": 0.0,
    "fluxo": 0.0,
}]
indice = 1.0  # indice da cota da carteira

dt_ant = "2025-12-31"
for dt in datas_cart:
    pat_hoje, pat_ontem_ajust, fluxo = snapshot_carteira(dt, dt_ant)
    # patrim base de ontem = patrim registrado em cart_hist[-1] (mais estavel)
    pat_ontem = cart_hist[-1]["patrimonio"]
    if pat_ontem > 0:
        # (pat_hoje - fluxo) representa o que o patrim_ontem virou sem o fluxo
        ret_dia = (pat_hoje - fluxo) / pat_ontem - 1
    else:
        ret_dia = 0.0
    indice *= (1 + ret_dia)
    ra = round((indice - 1) * 100, 6)
    vd = round(ret_dia * 100, 6)
    cart_hist.append({
        "dt": dt,
        "patrimonio": pat_hoje,
        "rent_acum": ra,
...
# Recalcula tot_rent (exibido nos cards) usando a rentabilidade TWR real,
# nao a soma bruta (patrim_fim/patrim_ini-1), que distorce com resgates.
tot_rent  = cart_rent
# tot_ganho continua sendo a variacao BRUTA de patrimonio (pode ser negativo
# se houve resgate liquido mesmo com rentabilidade positiva)
fluxo_total = round(sum(h.get("fluxo", 0.0) for h in cart_hist), 2)
tot_ganho   = round(tot_fim - tot_ini - fluxo_total, 2)  # ganho liquido de fluxo

print(f"  CARTEIRA (TWR)  R$ {tot_ini:,.2f} -> R$ {tot_fim:,.2f}  |  Rent real: {cart_rent:+.4f}%")
print(f"  Fluxo liquido no periodo: R$ {fluxo_total:+,.2f}  |  Ganho liquido: R$ {tot_ganho:+,.2f}\n")
...
def consolidar_grupo(nome, cor, cnpjs):
    ...
    resg_grupo = round(sum(
        h.get("resgate", 0.0)
        for c in cnpjs for f in [get_fundo(c)] if f
        for h in f["historico"] if h["dt"] <= ultima_dt
    ), 2)
    return {
        "resgates": resg_grupo,
...
<header>
  ...
  <div class="gerado">Gerado em """ + gerado_em + """ &nbsp;·&nbsp; Última publicação CVM (consolidado): """ + ultima_dt + """</div>
</header>
<main>
...
(function(){
  ...
  [
    ...
    {lbl:'Resgates no periodo',
     val: T.tem_resg ? fmtBRL(T.resg) : 'n.d.',
     sub: T.tem_resg ? 'fundos inteiros \\u00b7 todos os cotistas'
                     : 'RESG_DIA ausente no CSV',
     cls: ''},
  ].forEach(c=>{
...
// ── LEGENDA — CDI, Carteira Total, Lagunna e Grupo Neblina ───────────────────
...
function getDS(tab){
  ...
  const dsCart = [{
    ...
    data: CART.map(h=>h[key]),
    borderColor: TOTAIS.cart_color,
    backgroundColor: TOTAIS.cart_color+'18',
    fill: tab==='patrim',
  ...
  // Cards individuais podem ter datas diferentes (alguns ate 05/05, outros ate 30/04).
  // Aqui no grafico consolidado cortamos no ULTIMA_DT comum para nao misturar.
  const dsLagunna = lagunna ? [{
    label: lagunna.nome_curto,
    data: lagunna.historico.filter(h=>h.dt<=TOTAIS.dt_fim).map(h=>h[key]),
    borderColor: lagunna.color,
    backgroundColor: 'transparent',
    fill: false,
    borderWidth: 2.5, pointRadius: 0, tension: 0.1,
  }] : [];

  const dsGrupos = GRUPOS.map(g=>({
    label: g.nome_curto,
    data: g.historico.map(h=>h[key]),
    borderColor: g.color,
    backgroundColor: 'transparent',
    fill: false,
    borderWidth: 2.5, pointRadius: 0, borderDash: [3,3], tension: 0.1,
  }));

  const dsCDI = [{
    label: 'CDI',
    data: CDI.map(h=>h[key]),
    borderColor: TOTAIS.cdi_color,
    backgroundColor: 'transparent',
    fill: false,
    borderWidth: 2, pointRadius: 0, borderDash: [6,3], tension: 0.1,
  }];
...
DADOS.forEach((f,i)=>{
  ...
  card.innerHTML=`
    ...
    <div class="fc-row"><span class="fc-lbl">Resgates</span><span class="fc-val">${TOTAIS.tem_resg?fmtBRL(f.resgates):'n.d.'}</span></div>
...
GRUPOS.forEach((g,gi)=>{
  ...
  card.innerHTML=`
    ...
    <div class="fc-row"><span class="fc-lbl">Resgates</span><span class="fc-val">${TOTAIS.tem_resg?fmtBRL(g.resgates):'n.d.'}</span></div>
...
(function(){
  ...
  card.innerHTML=`
    ...
    <div class="fc-row"><span class="fc-lbl">Resgates</span><span class="fc-val">${TOTAIS.tem_resg?fmtBRL(T.resg):'n.d.'}</span></div>
...
function renderGrupoDet(g){
  ...
  detChart=new Chart(document.getElementById('detChart'),{
    type:'line',
    data:{labels:g.historico.map(h=>h.dt.slice(5)),datasets:[{label:g.nome,data:g.historico.map(h=>h.rent_acum),borderColor:g.color,backgroundColor:g.color+'18',fill:true,borderWidth:2,pointRadius:0,tension:0.1}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{autoSkip:true,maxTicksLimit:10,color:'#8892a8',font:{size:10}},grid:{color:'rgba(255,255,255,0.04)'}},y:{ticks:{color:'#8892a8',font:{size:10}},grid:{color:'rgba(255,255,255,0.04)'}}}},
  });
  const pc=g.rent>=0?'pos':'neg';
  document.getElementById('detMeta').innerHTML=[
    ...
    {lbl:'Resgates',val:TOTAIS.tem_resg?fmtBRL(g.resgates):'n.d.'},
...
function renderCartDet(){
  ...
  detChart=new Chart(document.getElementById('detChart'),{
    type:'line',
    data:{labels:CART.map(h=>h.dt.slice(5)),datasets:[{label:'Carteira',data:CART.map(h=>h.rent_acum),borderColor:TOTAIS.cart_color,backgroundColor:TOTAIS.cart_color+'18',fill:true,borderWidth:2.5,pointRadius:0,tension:0.1}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{autoSkip:true,maxTicksLimit:10,color:'#8892a8',font:{size:10}},grid:{color:'rgba(255,255,255,0.04)'}},y:{ticks:{color:'#8892a8',font:{size:10}},grid:{color:'rgba(255,255,255,0.04)'}}}},
  });
  const T=TOTAIS,pc=T.cart_rent>=0?'pos':'neg';
  document.getElementById('detMeta').innerHTML=[
    ...
    {lbl:'Resgates',val:T.tem_resg?fmtBRL(T.resg):'n.d.'},
...
function renderDet(f,tab){
  ...
  detChart=new Chart(document.getElementById('detChart'),{
    type:'line',
    data:{labels:f.historico.map(h=>h.dt.slice(5)),datasets:[{label:tab,data:f.historico.map(h=>h[key]),borderColor:f.color,backgroundColor:f.color+'18',fill:true,borderWidth:2,pointRadius:0,tension:0.1}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{autoSkip:true,maxTicksLimit:10,color:'#8892a8',font:{size:10}},grid:{color:'rgba(255,255,255,0.04)'}},y:{ticks:{color:'#8892a8',font:{size:10}},grid:{color:'rgba(255,255,255,0.04)'}}}},
  });
  const pc=f.rent>=0?'pos':'neg';
  document.getElementById('detMeta').innerHTML=[
    ...
    {lbl:'Resgates',val:TOTAIS.tem_resg?fmtBRL(f.resgates):'n.d.'},
...
(function(){
  ...
  new Chart(document.getElementById('pieChart'),{
    ...
    options:{responsive:true,maintainAspectRatio:false,cutout:'62%',plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>`  ${ctx.label}: ${fmtBRL(ctx.parsed)}`}}}},
  });
  ...
  document.getElementById('pieInfo').innerHTML=vals.map((v,i)=>{
    ...
    <div style="border-top:1px solid #1a2035;margin-top:8px;padding-top:8px">
      ...
      ${GRUPOS.map(g=>{const pc=g.rent>=0?'pos':'neg';return`<div class="pie-row" style="margin-bottom:6px">
