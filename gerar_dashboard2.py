...
# Divide cota por 1000 conforme solicitado

df[“VL_QUOTA_F”] = df[“VL_QUOTA_F”] / 1000.0
...
if cnpj_raw in cota_base:
    ...
    # patrimônio em R$ = patrim_liquido (já é o total do fundo, não dividimos)
    hist.append({
        "dt": "2025-12-31",
        "cota": round(cota_ini, 8),
        "cotas_qt": round(patrim_dez_val / (cota_ini * 1000), 6) if cota_ini else 0.0,
        "patrimonio": round(patrim_dez_val, 2),
        "rent_acum": 0.0,
        "var_diaria": 0.0,
    })
...
for dt in datas_todas:
    ...
    hist.append({
        ...
        "cotas_qt": qt, "patrimonio": round(pat, 2),
...
# Data limite = menor data final entre todos os fundos (data comum a todos)

ultima_dt = min(f[“dt_fim”] for f in fundos)
print(f”  Data limite comum: {ultima_dt}\n”)

# Ajusta patrimônio final de cada fundo para a data limite

for f in fundos:
h = next((x for x in reversed(f[“historico”]) if x[“dt”] <= ultima_dt), None)
if h:
f[“patrim_fim”] = h[“patrimonio”]
f[“cota_fim”]   = h[“cota”]
f[“dt_fim”]     = h[“dt”]
f[“rent”]       = round((h[“cota”] / f[“cota_ini”] - 1) * 100, 6) if f[“cota_ini”] else 0.0
f[“ganho”]      = round(h[“patrimonio”] - f[“patrim_ini”], 2)
...
// ── LEGENDA — só Lagunna e Grupo Neblina ─────────────────────────────────────
...
// ── GRAFICO PRINCIPAL — só Lagunna e Grupo Neblina ───────────────────────────
let mainChart;
...
function getDS(tab){
  ...
  const dsLagunna = lagunna ? [{
    ...
    data: lagunna.historico.map(h=>h[key]),
    borderColor: lagunna.color,
    backgroundColor: lagunna.color+'18',
    fill: tab==='patrim',
  ...
  const dsGrupos = GRUPOS.map(g=>({
    ...
    fill: tab==='patrim',
    borderWidth: 2.5, pointRadius: 0, borderDash: tab==='patrim'?[]:[3,3], tension: 0.1,
  }));
  return [...dsLagunna, ...dsGrupos];
}

function fmtY(tab,v){
  if(tab==='patrim') return 'R$'+Number(v/1000000).toFixed(1)+'M';
